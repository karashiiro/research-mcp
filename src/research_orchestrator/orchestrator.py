...
class ResearchOrchestrator:
    ...
    async def research_subtopic(self, subtopic: str, agent_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.research_logger.info(f"Research completed for '{subtopic}' by agent {agent_id}")
            
        except Exception as e:
            # Handle AI generation failures
            self.research_logger.error(f"Research generation failed for '{subtopic}': {e}")
            research_summary = {
                "message": {
    ...
    async def conduct_research(self, main_topic: str) -> Dict[str, Any]:
        ...
        master_synthesis = create_master_synthesis(main_topic, research_results, self.agent_manager.get_lead_researcher())
