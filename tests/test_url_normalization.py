...
from research_orchestrator.processing.citation_processor import CitationProcessor
...
class TestUrlNormalization:
    ...
    def test_basic_url_unchanged(self):
        ...
        assert self.processor.normalize_url(url) == "https://example.com/path"

    def test_trailing_slash_removed(self):
        """Test that trailing slashes are removed."""
        url = "https://example.com/path/"
        expected = "https://example.com/path"
        assert self.processor.normalize_url(url) == expected
    ...
    def test_invalid_url_fallback(self):
        ...
        assert self.processor.normalize_url(invalid_url) == "not-a-valid-url"
...
class TestCitationDeduplication:
    ...
    def test_no_sources_section(self):
        ...
        result = self.processor.deduplicate_citation_urls(text)
        assert result.updated_text == text
...
[5] Example Site – "Guide 3" – https://example.com/guide?ref=homepage
        ...
        assert "[2]" in result.updated_text
        assert "[3]" in result.updated_text
        assert "[4]" not in result.updated_text  # Should be consolidated
        assert "[5]" not in result.updated_text  # Should be consolidated

        # All Example Site references should point to [1]
        assert "citations [1] and [1] and [1]" in result.updated_text

        # Sources section should have only 3 unique URLs
        sources_lines = result.updated_text.split("## Sources")[1].strip().split("\n")
        actual_sources = [
...
[2] Example – "Same Guide" – https://example.com/guide/
        ...
        assert "[1]" in result.updated_text
        assert (
            "[2]" not in result.updated_text
            or "[2]" not in result.updated_text.split("## Sources")[1]
        )
        assert "Content with [1] and [1]" in result.updated_text
...
[1] Example – "Guide" – https://example.com/guide/
        ...
        assert "https://example.com/guide/" in result.updated_text
...
[7] Wiki Site – "Fourth Time" – https://wiki.example.com/x-guide#overview
        ...
        assert "referencing [1], [1], [1], and [1]" in result.updated_text

        # Final sources should have only unique URLs
        sources_section = result.updated_text.split("## Sources")[1]
